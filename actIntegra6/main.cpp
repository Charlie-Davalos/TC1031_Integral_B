...
int main() {
	...
	do {
		cout << "\nBienvenido a la tabla global de las emisiones de CO2 en el mundo, porfavor seleccione una opción\n";
		cout << "0. Salir\n";
		cout << "1. Imprimir pedazo de la tabla\n";
...
//casos prueba
		switch(answer){
			...
			case 2: {
				...
				cout << "Start row (min. 0) -> ";
				cin >> start;
				cout << "End row (max. 100) -> ";
				cin >> end;
				DList<DList<string>> sortedData = sortData(data, start, end, index);
				writeData(sortedData, "co2_emission_shuffled2.csv");
